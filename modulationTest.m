...
SRRC_MF_out = conv(out_PS_SRRC_withNoise,SRRC_MF); 
...
plot(ZF_HS)
title('Half sine modulated signal after channel, matched filter, and zero-forcing equalizer')
subplot(2,1,2)
plot(ZF_SRRC)   %plot SRRC
title('SRRC modulated signal after channel, matched filter, and zero-forcing equalizer')
% % Modulated signals after channel with noise, pass through equalizer
% ZF_HS_noise = ZF_equalizer(ch_coeff, fs, HS_MF_out);  % half sine
% ZF_SRRC_noise = ZF_equalizer(ch_coeff, fs, SRRC_MF_out);  % SRRC
% figure
% subplot(2,1,1)  %plot half sine
% plot(ZF_HS_noise)
